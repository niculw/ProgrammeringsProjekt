				...
				if ( (( ballXOffset + 1 ) >> 3 ) != ballXArray ) {		// så står der en blok til højre for bolden
					if ( (( ballY + 1 ) >> 1 ) != ballYArray ) {	// så står der OGSÅ en blok under bolden
						if ( ( blocks[ ballYArray ][ ballXArray + 1 ] & 0x01 == 0x01 )
							...
							updateBlock( &blocks[ ballYArray ][ ballXArray + 1 ], ballYArray , ballXArray + 1 );
							updateBlock( &blocks[ ballYArray + 1 ][ ballXArray ], ballYArray + 1 , ballXArray );
							...
							updateBlock( &blocks[ ballYArray + 1 ][ ballXArray + 1 ], ballYArray + 1 , ballXArray + 1 );
							return 8;	
						} else {
							return 0;
						}
